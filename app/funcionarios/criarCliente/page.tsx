...
export default function CadastroClientePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.message ?? 'Erro ao cadastrar cliente. Tente novamente.');
        return;
      }
      
      // Cliente criado com sucesso: volta para a listagem de clientes do dashboard
      router.push(`/funcionarios/${params.id}/dashboard`)
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-red-500/10 border border-red-500/30 rounded-xl flex items-center justify-center shrink-0">
            <UserPlus className="text-red-400 w-5 h-5" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white">Cadastrar Novo Cliente</h1>
            <p className="text-gray-400 text-sm">
              Preencha os dados abaixo para criar a conta do cliente
            </p>
          </div>
        </div>
      </div>
