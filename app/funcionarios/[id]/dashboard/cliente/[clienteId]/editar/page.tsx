...
import { Eye, EyeOff, AlertCircle, Loader2, UserCog, ArrowLeft } from 'lucide-react';
import { useRouter, useParams } from 'next/navigation';
import { getClienteParaEdicao, updateCliente } from './actions';
...
export default function EditarClientePage() {
  ...
  useEffect(() => {
    const carregar = async () => {
      try {
        setCarregando(true);
        const cliente = await getClienteParaEdicao(Number(clienteId));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await updateCliente(Number(clienteId), {
        nome: form.nome.trim(),
      ...
      if (!result.success) {
        setError(result.message ?? 'Erro ao atualizar cliente. Tente novamente.');
        return;
      }
      ...
      <div className="space-y-1">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">Atualize os dados do cliente</p>
          </div>
        </div>
      </div>
